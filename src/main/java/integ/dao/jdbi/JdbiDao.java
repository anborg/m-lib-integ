...
//    Long insert(@BindBean Model.Person in);
    ...
    interface acase {
        @Transactional
        @GetGeneratedKeys
        @SqlUpdate("insert into INTEG_CASE(id ,status ,type_id ,description) VALUES ( :id, :status, :typeId, :description )")
...
//        @RegisterBeanMapper(value = Model.Xref.class, prefix = "x")
        ...
        @Transactional
        @SqlUpdate("update INTEG_CASE set id = :id , status = :status , type_id = :typeId, address_id =:address.id, reportedby_person_id :=reportedByCustomer , createdby_emp_id := createdByEmployee, description  where id=cast(:id as INTEGER)")
    ...
    class ReducerCaseAddressXref implements LinkedHashMapRowReducer<Long, Model.Case.Builder> {
        ...
        public void accumulate(Map<Long, Model.Case.Builder> map, RowView rowView) {
            ...
            //Address for Person.
//            if (!p.hasEventAddress() && rowView.getColumn("c_address_id", Long.class) != null) {
//                Model.PostalAddress.Builder addr = rowView.getRow(Model.PostalAddress.Builder.class);
//                p.setEventAddress(addr);
//            }
//
//            //XREF for Person
//            String sysId = rowView.getColumn("x_xref_sys_id", String.class);
